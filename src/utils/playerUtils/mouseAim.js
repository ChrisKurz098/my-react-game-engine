...
export default function mouseAim(gameDim, setPlayer, kbCheck, msCheck, params) {
    setPlayer(old => {
        ...
        if (kbCheck.includes('d')) {  (x > gameDim.w) ? change.x = -w : change.x += spd; }
        if (kbCheck.includes('a')) {  (x < -(w)) ? change.x = gameDim.w - 1 : change.x -= spd; }
        if (kbCheck.includes('w')) {  (y < -(h)) ? change.y = gameDim.h - 1 : change.y -= spd; }
        if (kbCheck.includes('s')) {  (y > gameDim.h) ? change.y = -h : change.y += spd; }
